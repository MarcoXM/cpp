...
vector<string> LinuxParser::CpuUtilization() { 
  ...
  if (file_name.is_open()) {
    /*Stream the line into a buffer linestream*/
      std::istringstream linestream(line);
      while (linestream >> key >> user_time >>nice_time >> system_time>> idle_time >> iowait_time >> irq_time >> softirq_time>> steal_time >>guest_time>>guest_nice_time){
        if (key == "cpu"){
...
string LinuxParser::Command(int pid) { 
  ...
  std::string command;
  /* Get the file from the path */
  std::ifstream file_name(kProcDirectory + std::to_string(pid) + kCmdlineFilename);
  if (file_name.is_open()) {
...
string LinuxParser::Ram(int pid) { 
  ...
  /* Get the file from the path */
  std::ifstream file_name(kProcDirectory + std::to_string(pid) + kStatusFilename);
